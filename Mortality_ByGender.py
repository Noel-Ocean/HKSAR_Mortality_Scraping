...
def scraper_02(chrome_driver_path, data_website, year_to_select, cause_of_death):
    ...
    # Criteria Filtering #
    ################################################################################################################################
    print(f"Start data scraping for year={year_to_select}, cause of death={cause_of_death}, by district x gender...")
    ...
    try: 
        ...
        data_scraped = pd.DataFrame(
            ...
            columns=[f"{cause_of_death}_Male",f"{cause_of_death}_Female", f"{cause_of_death}_Unknown"],
        ...
        table_var_choose_one = table_dropdown_screening[1] ########################################################## 1=="Sex"
        ...
        reshaped_data = {
            f"{cause_of_death}_{index_list[-4]}": data_list[1:23],
            f"{cause_of_death}_{index_list[-3]}": data_list[23:45],
            f"{cause_of_death}_{index_list[-2]}": data_list[45:67]}
...
def scraper_02_extended (local_driver_path, hksar_page, list_of_diseases, year):
    result_list=[]
    for i in list_of_diseases:
        interim_result = scraper_02(
